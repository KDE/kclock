...
// lifted from libplasma, maybe this could move to KWindowSystem

// would need a flag to say all other clicks initiate a move
// we would want clicks for moves to be processed
// after the main window has handled it.. so maybe it needs different code.
...
WindowResizeHandler::WindowResizeHandler(QWindow *parent)
    : QObject(parent)
...
bool WindowResizeHandlerPrivate::updateMouseCursor(const QPointF &globalMousePos)
{
    ...
    // if (!sides) { // different from the libplasma version
    //     unsetMouseCursor();
    //     return false;
    // }
    ...
    if (sides == Qt::Edges(Qt::LeftEdge | Qt::TopEdge)) {
        ...
        window->setCursor(Qt::DragMoveCursor);
    }
